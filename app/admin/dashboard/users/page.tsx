          ...
          <div key={user.id} className="bg-white shadow-lg rounded-lg p-4 flex flex-col items-center">
            <div className="relative w-24 h-24 mb-4 rounded-full overflow-hidden">
              {user.profile_picture ? (
                <Image src={user.profile_picture} alt={`${user.firstName} ${user.lastName}`} layout="fill" objectFit="cover" />
              ) : (
            ...
            <p className={`text-sm ${user.status === UserStatus.ACTIVE ? 'text-green-500' : 'text-red-500'}`}>{user.status === UserStatus.ACTIVE ? 'Active' : 'Inactive'}</p>
            <p className="text-xs text-gray-500">{user.is_verified ? 'Verified' : 'Not Verified'}</p>
