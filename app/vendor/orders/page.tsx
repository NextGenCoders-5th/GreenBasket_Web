...
import { DraggableRow } from './components/DraggableRow';
 ...
 function OrderPage() {
  ...
  React.useEffect(() => {
    const handleTabChange = (event: KeyboardEvent) => {
      if (event.key === 'ArrowRight' || event.key === 'ArrowLeft') {
        event.preventDefault();
        setTab((prevTab) => {
          if (event.key === 'ArrowRight') {
            return prevTab === 'outline' ? 'past-performance' : prevTab === 'past-performance' ? 'key-personnel' : 'focus-documents';
          } else {
            return prevTab === 'focus-documents' ? 'key-personnel' : prevTab === 'key-personnel' ? 'past-performance' : 'outline';
          }
        });
      }
    };
  ...
  return (
    <Tabs defaultValue="outline" value={tab} className="w-full flex-col justify-start gap-6">
      <div className="flex items-center justify-between px-4 lg:px-6">
        <Label htmlFor="view-selector" className="sr-only">
          View
        </Label>
        <Select value={tab} onValueChange={(value) => setTab(value)} defaultValue="outline">
